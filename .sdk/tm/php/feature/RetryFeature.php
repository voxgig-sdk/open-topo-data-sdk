...
// OpenTopoData SDK retry feature
...
class OpenTopoDataRetryFeature extends OpenTopoDataBaseFeature
{
    ...
    public function init(OpenTopoDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenTopoDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenTopoDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenTopoDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
