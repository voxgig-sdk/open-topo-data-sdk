...
// OpenTopoData SDK pipeline test
//
...
{
    private static function utility(): OpenTopoDataUtility
    {
        return new OpenTopoDataUtility();
    }
    ...
    private static function ctx(array $over = []): OpenTopoDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenTopoDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenTopoDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenTopoDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenTopoDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenTopoDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenTopoDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenTopoDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenTopoDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenTopoDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenTopoDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenTopoDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenTopoDataResponse([]),
            'result' => new OpenTopoDataResult([]),
        ]);
        [, $err] = OpenTopoDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenTopoDataSpec([]),
            'response' => null,
            'result' => new OpenTopoDataResult([]),
        ]);
        [, $err] = OpenTopoDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenTopoDataSpec(['step' => 's']),
            'response' => new OpenTopoDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenTopoDataResult([]),
        ]);
        [, $err] = OpenTopoDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenTopoDataSpec(['step' => 's']),
            'response' => new OpenTopoDataResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenTopoDataResult([]),
        ]);
        [, $err] = OpenTopoDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenTopoDataSpec(['step' => 's']),
            'response' => new OpenTopoDataResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenTopoDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenTopoDataMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenTopoDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenTopoDataSpec([]),
            'response' => new OpenTopoDataResponse([]),
            'result' => new OpenTopoDataResult([]),
        ...
        [$got, $err] = OpenTopoDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenTopoDataResult([])]);
        [, $err] = OpenTopoDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenTopoDataSpec([]), 'result' => null]);
        [, $err] = OpenTopoDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenTopoDataSpec(['step' => 's']),
            'result' => new OpenTopoDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenTopoDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenTopoDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenTopoDataSpec(['step' => 's']),
            'result' => new OpenTopoDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenTopoDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenTopoDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenTopoDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenTopoDataSpec([]),
            'result' => new OpenTopoDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenTopoDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenTopoDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenTopoDataError('boom', 'boom');
        $utility->fetcher = function (OpenTopoDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenTopoDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenTopoDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenTopoDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenTopoDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenTopoDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OpenTopoDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenTopoDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenTopoDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenTopoDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenTopoDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenTopoDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenTopoDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenTopoDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenTopoDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenTopoDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenTopoDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenTopoDataResult(['ok' => false])]);
        $this->expectException(OpenTopoDataError::class);
        OpenTopoDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenTopoDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenTopoDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenTopoDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenTopoDataBaseFeature();
        $b = new OpenTopoDataBaseFeature();
        OpenTopoDataFeatureAdd::call($ctx, $a);
        OpenTopoDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenTopoDataBaseFeature
    {
        $f = new OpenTopoDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenTopoDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenTopoDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenTopoDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenTopoDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        OpenTopoDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenTopoDataResponse(['headers' => null]),
            'result' => new OpenTopoDataResult([]),
        ]);
        OpenTopoDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OpenTopoDataResultBody::call($ctx);
