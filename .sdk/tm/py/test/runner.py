...
class OpenTopoDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenTopoDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenTopoDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenTopoDataTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = OpenTopoDataTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenTopoDataTestRunner.getenv(key)
        ...
        explain = OpenTopoDataTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if OpenTopoDataTestRunner._test_control is not None:
            return OpenTopoDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenTopoDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenTopoDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenTopoDataTestRunner.load_test_control()
...
def load_env_local():
    OpenTopoDataTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return OpenTopoDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenTopoDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenTopoDataTestRunner.load_test_control()


def live_client_options():
    return OpenTopoDataTestRunner.live_client_options()


def live_delay_ms():
    return OpenTopoDataTestRunner.live_delay_ms()
