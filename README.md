...
Open Topo Data is a REST API server that provides access to elevation data.

Learn more about Open Topo Data at [www.opentopodata.org](https://www.opentopodata.org).
...
This SDK exposes the API as a small set of **semantic entities** — GetElevation — that you
...
const items = await client.GetElevation().list({ id: "example", location: "example" })
```
...
```go
client := sdk.Test()
result, err := client.GetElevation(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| **GetElevation** | The GetElevation entity (list). | `/{dataset}` |
...
$getelevations = $client->GetElevation()->list();
print_r(array_map(fn($item) => $item->data_get(), $getelevations));
```
