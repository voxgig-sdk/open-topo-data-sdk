...
boru-driven command-line client **and** interactive REPL for the OpenTopoData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-topo-data-cli)
make build

# 2. See usage (words, entities, env vars)
./open-topo-data-cli --help

# 3. Provide credentials once, via the environment
export OPEN_TOPO_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./open-topo-data-cli list get_elevation

# 5. Override the API base URL for a single call
OPEN_TOPO_DATA_BASE=https://api.example.com ./open-topo-data-cli list get_elevation

# 6. No arguments -> interactive REPL
./open-topo-data-cli
open-topo-data> list get_elevation
open-topo-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-topo-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/open-topo-data-cli list get_elevation
   ```
...
Run with no arguments to open a REPL (prompt `open-topo-data>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_elevation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/open-topo-data-cli`. |
...
(`./open-topo-data-cli <expr>`) and interactively (the REPL), and expressions compose the
