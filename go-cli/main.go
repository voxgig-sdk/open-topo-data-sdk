// Command OpenTopoData-cli is a CLI + REPL for the OpenTopoData SDK,
...
const prompt = "open-topo-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_elevation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OPEN_TOPO_DATA_APIKEY (api key), OPEN_TOPO_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OPEN_TOPO_DATA_APIKEY carries the API key and
	// OPEN_TOPO_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPEN_TOPO_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPEN_TOPO_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpenTopoDataSDK(opts)
