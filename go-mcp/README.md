...
[MCP](https://modelcontextprotocol.io) server exposing the OpenTopoData SDK as
two agent tools — `open-topo-data_list` and `open-topo-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-topo-data-mcp)
make build

# 2. Provide credentials via the environment
export OPEN_TOPO_DATA_APIKEY=sk_live_xxx
...
```jsonc
// open-topo-data_list: first page of records
{ "entity": "get_elevation" }
{ "entity": "get_elevation", "query": { } }

// open-topo-data_load: one record by id
{ "entity": "get_elevation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-topo-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user open-topo-data \
     -- "$PWD"/dist/*/open-topo-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `open-topo-data_list` and `open-topo-data_load` tools now appear
   in new sessions. Ask the agent to *"list get_elevation using open-topo-data"*
   and it calls `open-topo-data_list` with `{"entity":"get_elevation"}`.
...
```sh
export OPEN_TOPO_DATA_APIKEY=sk_live_xxx            # API key
export OPEN_TOPO_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `open-topo-data_list` tool
...
### Call the `open-topo-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `open-topo-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `open-topo-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPEN_TOPO_DATA_APIKEY` | API key sent with every request. |
| `OPEN_TOPO_DATA_BASE` | Optional override of the API base URL. |
