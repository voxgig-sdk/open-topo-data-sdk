...
func TestGetElevationEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GetElevation(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GetElevation(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OPEN_TOPO_DATA_TEST_GET_ELEVATION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		getElevationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_elevation")))
...
func get_elevationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OPEN_TOPO_DATA_TEST_GET_ELEVATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPEN_TOPO_DATA_TEST_GET_ELEVATION_ENTID"])
