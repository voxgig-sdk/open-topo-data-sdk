...
import (
	...
	omni "github.com/voxgig-sdk/open-topo-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenTopoDataError); is {
				out := map[string]any{
					"name":    "OpenTopoDataError",
...
func omniCtx(arg any, client *sdk.OpenTopoDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenTopoDataSDK); is {
			client = live
