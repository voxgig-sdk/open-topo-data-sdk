...
local OpenTopoDataSDK = {}
OpenTopoDataSDK.__index = OpenTopoDataSDK
...
OpenTopoDataSDK._make_feature = _make_feature


function OpenTopoDataSDK.new(options)
  local self = setmetatable({}, OpenTopoDataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function OpenTopoDataSDK:options_map()
...
function OpenTopoDataSDK:get_utility()
  return Utility.copy(self._utility)
end


function OpenTopoDataSDK:get_root_ctx()
  return self._rootctx
end


function OpenTopoDataSDK:prepare(fetchargs)
...
function OpenTopoDataSDK:direct(fetchargs)
...
-- by passing it.
function OpenTopoDataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function OpenTopoDataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "OpenTopoDataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function OpenTopoDataSDK:GetElevation(data)
  ...
  if data == nil then
    if self._get_elevation == nil then
      self._get_elevation = EntityMod.new(self, nil)
    end
...
function OpenTopoDataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = OpenTopoDataSDK.new(sdkopts)
  sdk.mode = "test"
