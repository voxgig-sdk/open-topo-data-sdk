...
// OpenTopoData SDK GetElevation entity
...
class GetElevationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetElevationEntity($this->_client, $opts);
    }

    /**
     * @param GetElevation|array $args GetElevation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OpenTopoDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetElevation|array The current GetElevation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetElevation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OpenTopoDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetElevation fields).
     */
     ...
     *
     * @param GetElevationListMatch|array|null $reqmatch Match filter (any subset
     *   of GetElevation fields) as an assoc-array; GetElevationListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetElevation[]|array A list of GetElevation items as assoc-arrays at
     *   the SDK boundary; throws OpenTopoDataError on failure (item-5 convention).
     */
