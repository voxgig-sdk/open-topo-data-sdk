...
class OpenTopoDataSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = OpenTopoDataHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = OpenTopoDataHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!OpenTopoDataFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, OpenTopoDataFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return OpenTopoDataUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = OpenTopoDataHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = OpenTopoDataHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = OpenTopoDataHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new OpenTopoDataError($op . "_allow",
                "OpenTopoDataSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = OpenTopoDataHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new OpenTopoDataError("graphql_error",
                "OpenTopoDataSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetElevation($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_elevation === null) {
                $this->_get_elevation = new GetElevationEntity($this, null);
            }
            return $this->_get_elevation;
        }
        return new GetElevationEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new OpenTopoDataSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
