...
class FtCtrl extends OpenTopoDataControl
{
...
class FtClient
{
    ...
    private OpenTopoDataUtility $utility;

    public function __construct(OpenTopoDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenTopoDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenTopoDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenTopoDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenTopoDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenTopoDataConfig::make_feature($name);
    ...
    private static function buildUrl(OpenTopoDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenTopoDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenTopoDataResponse($fetched);
        }
    ...
    private function fail(OpenTopoDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenTopoDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
