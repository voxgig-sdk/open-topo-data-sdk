...
class GetElevationDirectTest extends TestCase
{
    public function test_direct_list_get_elevation(): void
    {
        $setup = get_elevation_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-get_elevation", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["get_elevation01"] as $_liveKey) {
...
function get_elevation_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new OpenTopoDataSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
