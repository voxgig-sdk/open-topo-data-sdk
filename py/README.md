...
The SDK exposes the API as capitalised, semantic **Entities** — for example `client.GetElevation()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/open-topo-data-sdk/releases)) or
...
```python
try:
    getelevations = client.GetElevation().list({"id": "example", "location": "example"})
...
```python
get_elevations = client.GetElevation().list({"id": "example", "location": "example"})
```
...
```
py/
├── opentopodata_sdk.py         -- Main SDK module
