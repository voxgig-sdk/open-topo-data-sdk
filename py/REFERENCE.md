# OpenTopoData Python SDK Reference

Complete API reference for the OpenTopoData Python SDK.
...
#### `OpenTopoDataSDK.test(testopts=None, sdkopts=None)`
...
#### `GetElevation(data=None)`

Create a new `GetElevationEntity` instance. Pass `None` for no initial data.
...
```python
results = client.GetElevation().list({"id": "example", "location": "example"})
...
Create a new `GetElevationEntity` instance with the same options.
