...
from opentopodata_sdk.core.helpers import get_ctx_prop, to_map
...
class OpenTopoDataContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, OpenTopoDataSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, OpenTopoDataResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, OpenTopoDataResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return OpenTopoDataError(code, msg, self)
