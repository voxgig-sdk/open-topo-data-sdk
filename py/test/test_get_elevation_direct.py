...
class TestGetElevationDirect:

    def test_should_direct_list_get_elevation(self):
        setup = _get_elevation_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-get_elevation", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["get_elevation01"]:
...
def _get_elevation_direct_setup(mockres):
