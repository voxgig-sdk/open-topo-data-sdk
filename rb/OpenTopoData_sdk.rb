...
require_relative 'OpenTopoData_types'
...
class OpenTopoDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpenTopoDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpenTopoDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpenTopoDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpenTopoDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpenTopoDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpenTopoDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpenTopoDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = OpenTopoDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpenTopoDataError.new(
        "graphql_error", "OpenTopoDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OpenTopoDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
