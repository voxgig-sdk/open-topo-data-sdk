...
class GetElevationEntity
  ...
  def make
    opts = @_entopts.dup
    GetElevationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OpenTopoDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = OpenTopoDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [GetElevationListMatch, Hash, nil] match filter (any subset of
  #   GetElevation fields); defaults to nil, treated as an empty match that lists all.
