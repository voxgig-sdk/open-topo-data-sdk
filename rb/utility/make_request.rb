...
module OpenTopoDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenTopoDataResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenTopoDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenTopoDataResponse.new(fetched)
    else
