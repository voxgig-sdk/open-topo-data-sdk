...
(0, node_test_1.describe)('GetElevationDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.OpenTopoDataSDK({
    ...
    (0, node_test_1.test)('direct-list-get_elevation', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-get_elevation', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["get_elevation01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            query.location = "56.35,123.90";
...
function liveScenariosActive() { return false && process.env.OPEN_TOPO_DATA_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.OpenTopoDataSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['OPEN_TOPO_DATA_TEST_GET_ELEVATION_ENTID'];
    ...
    const client = new __1.OpenTopoDataSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=GetElevationDirect.test.js.map