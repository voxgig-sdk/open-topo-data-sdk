...
describe('GetElevationDirect', async () => {
  ...
  test('direct-list-get_elevation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-get_elevation', setup.live)) return
    if (skipIfMissingIds(t, setup, ["get_elevation01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OPEN_TOPO_DATA_TEST_GET_ELEVATION_ENTID']
