...
//     (\`new OpenTopoDataSDK(...)\` / \`OpenTopoDataSDK.test(...)\` become
//     \`OpenTopoDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OpenTopoDataSDK'
...
const TEST_SEED = {"entity":{"get_elevation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpenTopoDataSDK(...)\` and any
// \`OpenTopoDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenTopoDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenTopoDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
